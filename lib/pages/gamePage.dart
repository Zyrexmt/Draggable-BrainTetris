...
import 'package:modulo_a1_emerson/services/tetrisLogic.dart';
...
class _GamePageState extends State<GamePage> {
  ...
  final String _playerName = nomeJogador;
  ...
  void _startCountdown() {
    Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      if(_countdown==1) {
        ...
        Provider.of<TetrisGameProvider>(context, listen: false).startGame();
      } else {
        setState(() => _countdown--,);
      }
    });
  ...
  void _onDragUpdate(DragUpdateDetails details, TetrisGameProvider game) {
  ...
  Future<void> saveAndExit(int score) async {
    ...
    if(existing!=null) {
      ...
      list.add(RankingEntry(playerName: _playerName.trim(), score: score));

      await prefs.setString('ranking', jsonEncode(list.map((e) => e.toJson()).toList()));
  ...
  void onEncerrar() {
    final game = Provider.of<TetrisGameProvider>(context, listen: false);
    game.stopGame();
    Navigator.pushReplacementNamed(context, '/home');
  }
        ...
        child: SafeArea(
          child: Consumer<TetrisGameProvider>(
            builder: (context, game, _) {
              if (_gameStart && game.isGameOver && !_saved) {
                _saved = true;
                Future.microtask(() {
